    ...
    <div className="space-y-8">
      ...
      {categories.length > 0 && (
        <div className="mb-8 flex flex-wrap gap-2">
          <Link
            href={tipo ? `/inicio?tipo=${tipo}` : "/inicio"}
          ...
          {categories.map((c) => (
            <Link
              key={c.id}
              href={tipo ? `/inicio?tipo=${tipo}&categoria=${c.slug}` : `/inicio?categoria=${c.slug}`}
              className={`rounded-full px-4 py-1.5 text-xs font-medium transition-colors ${
                categoria === c.slug
                  ? "bg-tech-blue text-white"
                  : "bg-app-chip text-cloud-white/70 hover:bg-app-chip-accent"
          ...
          {contentWithCategory.map((item) => {
            ...
            return (
              <Link
                key={item.id}
                href={href}
                className="group flex items-center gap-4 rounded-xl border border-app-border bg-midnight-blue px-4 py-3 transition-all hover:border-cyan-ia/50 hover:shadow-md"
              >
                {/* Thumbnail */}
                <div className="relative h-[68px] w-[120px] shrink-0 overflow-hidden rounded-lg">
                  {item.thumbnail ? (
                    ...
                    <div className={`flex h-full w-full items-center justify-center rounded-lg ${isVideo ? 'bg-gradient-to-br from-indigo-900/60 to-purple-900/60' : 'bg-deep-navy border border-app-border'}`}>
                      {isVideo ? (
                ...
                <div className="hidden shrink-0 flex-col items-end gap-1 sm:flex">
                  ...
                  <span className="text-[10px] text-cloud-white/40">
                    {item.publishedAtDate.toLocaleDateString('pt-BR', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </span>
                </div>
              </Link>
            );
